...
public class Okno extends JDialog {
	...
	static final boolean consoleToFile = false;
	static Uprawnienia uprawnienia;
	...
	public static void main(String[] args) {
		try {
			Logowanie.pokaz();
			if (Logowanie.uprawnienia == null)
				return;
			if (consoleToFile) {
				PrintStream CombinedOutput = new DualStream(System.out,
						new PrintStream(new BufferedOutputStream(new FileOutputStream("out.log", true))));
				System.setOut(CombinedOutput);
			}

			System.out.println("---------------------------------");
			String timeStamp = new SimpleDateFormat("dd-MM-yyyy HH:mm:ss").format(Calendar.getInstance().getTime());
			System.out.println("Start systemu: " + timeStamp);
	...
	public Okno() {
		...
		}
		{
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton_html.setVisible(Logowanie.uprawnienia.isExportHtml());
			...
			}
			{
				...
				btnExcel.setVisible(Logowanie.uprawnienia.isExportExcell());
			}
			{
				...
				btnNewButton_usun.setVisible(Logowanie.uprawnienia.isDelete());
			}
			{
				...
				btnNewButton_add.setVisible(Logowanie.uprawnienia.isAdd());
			...
			}
			{
				...
				btnNewButton_zmien.setVisible(Logowanie.uprawnienia.isEdit());
				{
					JButton btnNewButton_Admin = new JButton("Admin");
					buttonPane.add(btnNewButton_Admin);
					btnNewButton_Admin.setVisible(Logowanie.uprawnienia.isAdmin());
				}
	...
	void pobierz() {
		...
		if (this.comboBox_Sep.getSelectedItem().equals("Sep"))
			...
			sep = 0;
		String sql = " select p.id, p.imie, p.nazwisko, p.wiek, s.nazwa, p.sep, !isnull(zdjecie) czyzdj ";
...
class MyTableModel extends AbstractTableModel {
	private String[] columnNames = { "Lp", "Id", "Imię", "Nazwisko", "Wiek", "Stanowisko", "Sep", "Zdjecie" };
	public Object[][] data;
